...
class Dpr(BaseEmbeddingModel):
    ...
    def load_model(self):
        ...
        tokenizer = DPRContextEncoderTokenizer.from_pretrained(self.model_path)
        model = DPRContextEncoder.from_pretrained(self.model_path)
        model.eval()
    ...
    def sentence_to_embedding(self, sentences: List[str]):
        ...
        """
        with torch.no_grad():
            input_ids = self.tokenizer(sentences, padding=True, truncation=True, return_tensors="pt", max_length=512)["input_ids"].to(self.device)
...
if __name__ == "__main__":
    # only for test, please come to ../VDB_construct.py to run the real code
    dragon_plus = Dpr(
        ...
        save_path_prefix="data/corpus/vector_database/dpr/medical_with_answer",
        is_test=True
    )
    embeds = dragon_plus.sentence_to_embedding(["hello world", "goodbye world"])
    print(embeds.shape)
