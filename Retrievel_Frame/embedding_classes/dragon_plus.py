...
class DragonPlus(BaseEmbeddingModel):
    ...
    def sentence_to_embedding(self, sentences: List[str]):
        ...
        with torch.no_grad():
            encoded_sentences = self.tokenizer(sentences, padding=True, truncation=True, return_tensors='pt',
                                               max_length=512)
            encoded_sentences = {k: v.cuda(self.device) for k, v in encoded_sentences.items()}
...
if __name__ == "__main__":
    ...
    dragon_plus = DragonPlus(
        ...
        save_path_prefix="data/corpus/vector_database/dragon_plus/medical_with_answer",
        is_test=True
    )
